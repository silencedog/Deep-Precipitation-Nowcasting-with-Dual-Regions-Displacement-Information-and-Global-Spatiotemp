...
class LSTMCell(nn.Module):
    def __init__(self, in_channel, num_hidden, height, width, filter_size, stride, layer_norm):
        ...
        self.vit = VisionTransformer(img_size=32, patch_size=16, in_chans=64)

    def forward(self, x_t, h_t, c_t, m_t, motion_highway):
        h_t, c_t, m_t = h_t.cuda(), c_t.cuda(), m_t.cuda()
        ...
        # st_vit
        st_vit = self.vit(m_new_new)
        o_t = torch.sigmoid(o_x + o_h + self.conv_o(mem))
        h_new = o_t * torch.tanh(st_vit) + (1 - o_t) * motion_highway
